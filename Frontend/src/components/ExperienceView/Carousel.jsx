          ...
          <div className="flex flex-col justify-center items-center">
            ...
            <button onClick={prevImage} className="md:px-4 py-2  text-black font-bold transition duration-300"> <FontAwesomeIcon icon={faChevronLeft} /> </button>
              {displayedImages.map((url, index) => (
                <img key={index} src={url} alt={`Imagen ${currentImageIndex + index + 1}`} className={`${window.innerWidth < 640 && index > 0 ? 'hidden' : 'block'} md:w-1/4 w-full h-auto object-cover rounded-2xl`} />
              ))}
